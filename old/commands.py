...
def get_price(card):
    try:
        ...
        if discount_price:
            discount_price = ''.join(c for c in discount_price if c.isdigit())
        if main_price:
            main_price = ''.join(c for c in main_price if c.isdigit())
        if old_price:
            old_price = ''.join(c for c in old_price if c.isdigit())
        if price_per_m2:
            price_per_m2 = ''.join(c for c in price_per_m2 if c.isdigit())
